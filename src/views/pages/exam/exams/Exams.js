...
const Exams = () => {
  ...
  if (typeExam !== 'Tất cả') {
    ...
    exams.forEach(function (value) {
      if (value.typeCategory === typeExam)
        examWithFilter.push(value);
    });
  }
  else {
  ...
  var examWithFilterPage = examWithFilter.slice((currentPage - 1) * itemPerPage, currentPage * itemPerPage);
  ...
  return (
    <div className="exam-wrapper">
      <Container className="exam">
        <Filter setTypeExam={setTypeExam} typeExam={typeExam} setSortExam={setSortExam} />
        <div className="list-exam">
          {examWithFilterPage.map((val, idx) => (
            <ExamCard key={idx} ExamInfomation={val} />
          ))}
        </div>
        <PaginationOutlined currentPage={currentPage} setCurrentPage={setCurrentPage} index={examWithFilter.length} itemPerPage={itemPerPage}></PaginationOutlined>
      </Container>
    </div>
  );
};
