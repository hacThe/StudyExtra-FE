...
import BackToPageButton from "../../components/BackToPageButton.js";
function ConcreteDocument() {
  ...
  const currentDocument =
    useSelector((state) => {
      return state.document.currentViewingDoc;
    }) || [];
  ...
  return (
    <div className="concrete-document-page-container concrete-document">
      <div className="document-container">
        <div className="document-body">
          <BackToPageButton
            ...
            content={"Về trang tài liệu"}
          ></BackToPageButton>

          <div style={{ textAlign: "left" }} className="document-title">
          ...
          <div className="document-information">
            <div className="information-row">
              ...
              <div className="document-time">
                Ngày đăng:{" "}
                {convertStringToReadableDate(currentDocument.createdAt)}(
                {currentDocument.updatedAt
                  ? calculateTime(currentDocument.createdAt)
                  : "?"}
                )
              </div>
            </div>
            <div className="information-row-2">
              <div className="document-type">Loại:</div>
              <div className="document-type-container">
                {documentTypes.map((value, index, key) => {
                  if (
                    currentDocument.typeID &&
                    currentDocument.typeID.includes(value._id)
                  )
                    return <div className="document-item">{value.name}</div>;
                })}
              </div>
            </div>
          </div>
          <div className="document-iframe">
            <iframe
              src={
                ...
                currentDocument.link.indexOf("https:") != -1
                  ? currentDocument.link
                  : "https://" + "notfound"
              }
              width="100%"
