            ...
            <select
              ...
              onChange={(e) => {
                ...
                switch (e.target.value) {
                  case "Mới nhất":
                    console.log(
                      "documentWithFilter before sort",
                      documentWithFilter
                    );
                    documentWithFilter.sort(function (a, b) {
                      return new Date(b.createdAt) - new Date(a.createdAt);
                    });
                    console.log(
                      "documentWithFilter after sort",
                      documentWithFilter
                    );
                    break;
                  case "Cũ nhất":
            ...
            {documents.length === 0
              ...
              : documents.map((val, index) =>
                  index < page * maxItem && index >= (page - 1) * maxItem ? (
                    val.isHidden ? null : typeof documentTypes[
                        currentTypeSelect
                      ] != "undefined" &&
                      documentTypes[currentTypeSelect]._id == "abc" ? (
                      <DocumentCard
                        author={val.author}
                        name={val.name}
                        views={val.views}
                      ...
                      />
                        ...
                        documentTypes[currentTypeSelect]._id
                      ) ? (
                      <DocumentCard
                        author={val.author}
                        name={val.name}
