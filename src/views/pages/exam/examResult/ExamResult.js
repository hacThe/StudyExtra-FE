        ...
        <Container className="exam-result-wrapper" maxWidth="xl">
            <Grid container spacing={2} className="exam-grid">
                                ...
                                {Questions[idx].listAnswers.map((item, index) => (
                                    <div key={index}>
                                        <label className="question-content">
                                            {val.rightAnswer === index && <GoCheck style={{color: '#7B68EE'}}/>}
                                            {takeExam.resultExam.userAnswer[idx] === (index + 1) && takeExam.resultExam.userAnswer[idx] !== (val.rightAnswer + 1) && <TiDeleteOutline style={{color: "#C30000"}}/>}
                                            {val.rightAnswer !== index && takeExam.resultExam.userAnswer[idx] !== (index + 1) && <GoCheck style={{color: 'transparent'}} />}
                ...
                <Grid item xs={12} md={5} lg={4} className="leader-board_column">
                    <Button variant="contained" className="btn-contained" onClick={() => takeExamHandleClick()}>Thi lại</Button>
