...
function EditAnnouncement(props) {
  ...
  useEffect(() => {
    const fetApi = async () => {
      await axios
        .get(URL.URL_GET_ANNOUNCEMENT + slug)
        .then((res) => {
          if (res.status == 200) {
  ...
  const handleUpdateAnnouncement = async () => {
    await axios
      .post(URL.URL_UPDATE_ANNOUNCEMENT, {
        slug,
        content,
        title,
      })
  ...
  const handleDeleteAnnouncement = async () => {
    await axios
      .post(URL.URL_DELETE_ANNOUNCEMENT, {
        slug,
      })
      .then((res) => {
        if (res.data.status == 200) {
    ...
    <div className="add-announcement">
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <div
          ...
          style={{ display: "flex", color: " #747474", cursor: "pointer" }}
        >
          <BsArrow90DegLeft
            fontSize={20}
            style={{ marginRight: "10px" }}
          ></BsArrow90DegLeft>
          <p style={{ lineHeight: "17px", fontSize: "14px" }}>
            Danh sách thông báo
          </p>
        </div>
      ...
      <Grid
        style={{ marginTop: "20px", height: "90%", overflow: "hidden" }}
        container
        spacing={2}
      >
        <Grid style={{ paddingLeft: "20px" }} sm={6}>
          <div className="name">Tên thông báo</div>
          <input
            ...
            type="text"
          ></input>
          <div className="name">Nội dung thông báo</div>
          <div style={{ paddingRight: "10px" }}>
            <div className="create-content">
              <SunEditor
                ...
                autoFocus={true}
                setContents={content == "" ? review : content}
        ...
        <Grid sm={6}>
          <div className="name">Xem trước</div>
